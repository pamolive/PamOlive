...
from cbpam.audit.services import _canonical_payload, _payload_hash, record_event
from cbpam.common.keyring import get_keyring_client
from cbpam.vault.key_rotation import ENCRYPTED_MODEL_FIELDS
from cbpam.vault.legacy import LegacyVaultCipher
...
class Command(BaseCommand):
    ...
    def _run(self, *, apply):
        ...
        legacy_audit_key = os.environ.get("CBPAM_AUDIT_SIGNING_KEY", "").encode()
        if not legacy_audit_key:
            raise CommandError("CBPAM_AUDIT_SIGNING_KEY is required for audit migration")
