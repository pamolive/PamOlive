...
class KeyringClient:
    def __init__(self, base_url=None, timeout=None):
        self.base_url = (base_url or settings.PAMOLIVE_KEYRING_URL).rstrip("/")
        self.timeout = timeout or settings.PAMOLIVE_KEYRING_TIMEOUT_SECONDS
...
class LocalTestKeyringClient:
    ...
    def __init__(self):
        ...
        self._cipher = Fernet(settings.PAMOLIVE_TEST_KEYRING_ENCRYPTION_KEY.encode())
        self._signing_key = settings.PAMOLIVE_TEST_KEYRING_SIGNING_KEY.encode()
...
def get_keyring_client():
    backend = settings.PAMOLIVE_KEYRING_BACKEND
    if backend == "http":
