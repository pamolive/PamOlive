...
from cbpam.gateway.crypto import GatewayProtocolError, encrypt_envelope
...
def test_gateway_config_requires_distinct_long_keys(monkeypatch, tmp_path):
    monkeypatch.setenv("CBPAM_GATEWAY_SHARED_KEY", "s" * 40)
    monkeypatch.setenv("CBPAM_RECORDING_KEY", "r" * 40)
    monkeypatch.setenv("CBPAM_RECORDING_DIR", str(tmp_path))
    monkeypatch.setenv("CBPAM_INTERNAL_URL", "http://web.test/")
    monkeypatch.setenv("CBPAM_GATEWAY_CONNECT_TIMEOUT", "4.5")
    ...
    monkeypatch.setenv("CBPAM_RECORDING_KEY", "short")
...
def test_internal_api_client_signs_authorization_and_decrypts_envelope(monkeypatch):
    ...
    monkeypatch.setattr("cbpam.gateway.client.urllib.request.urlopen", fake_urlopen)
...
def test_internal_api_client_handles_unavailable_close_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("cbpam.gateway.client.urllib.request.urlopen", unavailable)
    client = InternalAPIClient(config)
...
@override_settings(
    CBPAM_GATEWAY_CONTROL_URL="http://gateway.internal",
    CBPAM_GATEWAY_SHARED_KEY="s" * 40,
)
def test_termination_control_returns_gateway_acknowledgement(monkeypatch):
    ...
    monkeypatch.setattr("cbpam.sessions.control.urllib.request.urlopen", accepted)
...
def test_request_client_ip_only_trusts_proxy_header_when_configured():
    ...
    with override_settings(CBPAM_TRUST_PROXY_HEADERS=False):
        assert request_client_ip(request) == "127.0.0.1"
    with override_settings(CBPAM_TRUST_PROXY_HEADERS=True):
